-- Vuelve `registrar_venta` al cuerpo anterior (sin `es_venta_libre` en el
-- INSERT de renglones) y saca la columna. OJO: los renglones libres ya
-- registrados quedan como "producto eliminado" al perder la marca; la plata
-- de esas ventas no se toca.
...
alter table public.ventas_items drop column if exists es_venta_libre;
